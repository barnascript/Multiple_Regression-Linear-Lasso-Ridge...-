...
class EngineClass:
    ...
    def main(self, type_="linear"):
        if type_ == "linear":
            linear_model = LinearRegression(lr=0.00001, n_iter=100, csv_path=self.csv_path)
            linear_model.LR_main()
            #Saving model in Output Folder
            pkl.dump(linear_model, open(self.model_save_path+"linear_model.pkl", "wb"))

        elif type_ == "lasso":
            lasso_model = LassoRegression(alpha=0.03, lr=0.0001, n_iter=100, csv_path=self.csv_path)
            lasso_model.LR_main()
            #Saving model in Output Folder
            pkl.dump(lasso_model, open(self.model_save_path+"lasso_model.pkl", "wb"))

        elif type_ == "ridge":
            ridge_model = RidgeRegression(alpha=0.03, lr=0.0001, n_iter=100, csv_path=self.csv_path)
            ridge_model.RR_main()
            #Saving model in Output Folder
            ...
            #Saving model in Output Folder
            pkl.dump(root_node, open(self.model_save_path+"regression_tree_model.pkl", "wb"))


if __name__ == "__main__":
    csv_path = "../inputFiles/Football_Data.csv"
